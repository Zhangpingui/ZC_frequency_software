...
def build_conflict_pair_figure(records, page=1, page_size=20, only_conflicts=False) -> go.Figure:
        ...
        for row, record in enumerate(visible):
            ...
            if _is_conflict(record):
                ...
                figure.add_annotation(
                    x=0.5, y=y_position + 0.16, text=str(record.conflict_type),
                    showarrow=False, font={"color": "#ff3344", "size": 11},
                )
    ...
    figure.update_layout(
        ...
        xaxis={"tickmode": "array", "tickvals": [0, 1], "ticktext": ["链路一", "链路二"], "range": [-0.35, 1.35]},
        yaxis={"title": "记录", "showticklabels": False, "range": [0.4, max(1.6, len(visible) + 0.8)]},
